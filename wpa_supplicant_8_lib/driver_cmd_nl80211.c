...
static int wpa_driver_set_backgroundscan_params(struct i802_bss *bss, const char *cmd)
{
    /*
	struct i802_bss *bss = priv;
	struct wpa_driver_nl80211_data *drv = bss->drv;
				  ...
				  size_t buf_len )
{
	...
	} else if (os_strncasecmp(cmd, "BGSCAN-START", 12) == 0) {
        ret = wpa_driver_set_backgroundscan_params(bss, cmd);
        if (ret < 0) {
